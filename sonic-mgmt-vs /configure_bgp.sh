...
# 1 DUT (Device Under Test) connected to 4 Leaf nodes
...
LEAF_CONTAINERS=("clab-sonic-vs-t0-leaf1" "clab-sonic-vs-t0-leaf2" "clab-sonic-vs-t0-leaf3" "clab-sonic-vs-t0-leaf4")
...
configure_dut_bgp() {
    ...
    local neighbor3="10.0.0.9"   # leaf3
    local neighbor4="10.0.0.13"  # leaf4
    ...
    docker exec $container_name vtysh -c "configure terminal" \
        ...
        -c "neighbor $neighbor3 remote-as 65003" \
        -c "neighbor $neighbor3 description leaf3" \
        -c "neighbor $neighbor4 remote-as 65004" \
        -c "neighbor $neighbor4 description leaf4" \
...
configure_leaf_bgp() {
    ...
    if [ "$container_name" == "clab-sonic-vs-t0-leaf1" ]; then
        ...
        neighbor1_desc="dut"
    elif [ "$container_name" == "clab-sonic-vs-t0-leaf3" ]; then
        router_id="10.10.10.3"
        neighbor1="10.0.0.8"  # dut
        neighbor1_desc="dut"
    else
        router_id="10.10.10.4"
        neighbor1="10.0.0.12"  # dut
        neighbor1_desc="dut"
    fi
...
# DUT interfaces (connected to leaves 1-4) - using vtysh
docker exec $DUT_CONTAINER vtysh -c "configure terminal" \
    ...
    -c "ip address 10.0.0.8/31" \
    -c "no shutdown" \
    -c "exit" \
    -c "interface Ethernet12" \
    -c "ip address 10.0.0.12/31" \
    -c "no shutdown" \
    -c "exit" \
...
# Leaf 3 interfaces
docker exec clab-sonic-vs-t0-leaf3 vtysh -c "configure terminal" \
    -c "interface Ethernet0" \
    -c "ip address 10.0.0.9/31" \
    -c "no shutdown" \
    -c "exit" \
    -c "interface Loopback0" \
...
# Leaf 4 interfaces
docker exec clab-sonic-vs-t0-leaf4 vtysh -c "configure terminal" \
    -c "interface Ethernet0" \
    -c "ip address 10.0.0.13/31" \
    -c "no shutdown" \
    -c "exit" \
    -c "interface Loopback0" \
    -c "ip address 10.10.10.4/32" \
    -c "exit" 2>&1 | grep -v "Unknown command" || true
...
configure_leaf_bgp "clab-sonic-vs-t0-leaf3" "65003"
configure_leaf_bgp "clab-sonic-vs-t0-leaf4" "65004"
