...
docker exec "$DUT_CONTAINER" bash -c "mkdir -p /etc/sudoers.d && echo 'admin ALL=(ALL) NOPASSWD:ALL' > /etc/sudoers.d/admin && chmod 0440 /etc/sudoers.d/admin"

# Enable password authentication in SSH
log_info "Enabling password authentication in SSH..."
docker exec "$DUT_CONTAINER" bash -c "sed -i 's/#PasswordAuthentication yes/PasswordAuthentication yes/' /etc/ssh/sshd_config"

# Set root password for SSH access
log_info "Setting root password..."
docker exec "$DUT_CONTAINER" bash -c "echo 'root:sonic' | chpasswd"

# Set platform in config_db if not already set
log_info "Setting platform in config_db..."
docker exec "$DUT_CONTAINER" bash -c "python3 << 'PYEOF'
import json
with open('/etc/sonic/config_db.json', 'r') as f:
    config = json.load(f)
if 'platform' not in config['DEVICE_METADATA']['localhost']:
    config['DEVICE_METADATA']['localhost']['platform'] = 'x86_64-kvm_x86_64-r0'
    with open('/etc/sonic/config_db.json', 'w') as f:
        json.dump(config, f, indent=4)
    print('Platform set in config_db')
else:
    print('Platform already set in config_db')
PYEOF
"

# Set build_version in sonic_version.yml if empty
log_info "Setting build_version in sonic_version.yml..."
docker exec "$DUT_CONTAINER" bash -c "python3 << 'PYEOF'
import yaml
with open('/etc/sonic/sonic_version.yml', 'r') as f:
    version = yaml.safe_load(f)
if not version.get('build_version'):
    version['build_version'] = '202305.978469'
    with open('/etc/sonic/sonic_version.yml', 'w') as f:
        yaml.dump(version, f)
    print('Build version set to 202305.978469')
else:
    print('Build version already set')
PYEOF
"

# Add has_per_asic_scope to FEATURE table in config_db
log_info "Adding has_per_asic_scope to FEATURE table..."
docker exec "$DUT_CONTAINER" bash -c "python3 << 'PYEOF'
import json
with open('/etc/sonic/config_db.json', 'r') as f:
    config = json.load(f)
if 'FEATURE' in config:
    for service in config['FEATURE']:
        if 'has_per_asic_scope' not in config['FEATURE'][service]:
            config['FEATURE'][service]['has_per_asic_scope'] = 'False'
    with open('/etc/sonic/config_db.json', 'w') as f:
        json.dump(config, f, indent=4)
    print('Added has_per_asic_scope to all features')
else:
    print('FEATURE table not found in config_db')
PYEOF
"
