...
declare -r LOCAL_IMAGE_NAME="docker-sonic-mgmt-$(echo "${CURRENT_USER}" | tr '[:upper:]' '[:lower:]')"
...
# Try to get docker group ID, fallback to 999 if not found (common docker group ID)
_DGID=$(dscl . -read /Groups/docker PrimaryGroupID 2>/dev/null | awk '{print $2}')
declare -r HOST_DGID="${_DGID:-999}"
...
IMAGE_ID=""
LINK_DIR=""
# Use rslave mount option on Linux, but not on Docker Desktop (macOS or Windows)
# Docker Desktop doesn't support rslave mount option
if docker info 2>/dev/null | grep -q "Operating System: Docker Desktop"; then
    MOUNT_POINTS="-v \"/var/run/docker.sock:/var/run/docker.sock\""
else
...
function show_help_and_exit() {
    ...
    echo
    echo "Other options:"
    echo "  -u <username>        set the username for the container (default: current user from whoami)"
    ...
    echo
    echo "Examples:"
    echo "  ./${SCRIPT_NAME} -n sonic-mgmt-test"
    echo "  ./${SCRIPT_NAME} -n sonic-mgmt-test -u rwang"
    echo "  ./${SCRIPT_NAME} -n sonic-mgmt-test -i sonicdev-microsoft.azurecr.io:443/docker-sonic-mgmt:latest"
    echo "  ./${SCRIPT_NAME} -n sonic-mgmt-test -d /var/src"
    echo "  ./${SCRIPT_NAME} -n sonic-mgmt-test -m /my/working/dir"
    echo "  ./${SCRIPT_NAME} -n sonic-mgmt-test -p 192.0.2.1:8080:80/tcp"
    echo "  ./${SCRIPT_NAME} -n sonic-mgmt-test --enable-debug"
    echo "  ./${SCRIPT_NAME} -h"
    echo
    exit ${1}
}
...
function show_local_container_login() {
    ...
    echo "******************************************************************************"
    echo "EXEC: docker exec --user ${CONTAINER_USER} -ti ${CONTAINER_NAME} bash"
    echo "SSH:  ssh -i ~/.ssh/id_rsa_docker_sonic_mgmt ${CONTAINER_USER}@${CONTAINER_IPV4}"
    echo "******************************************************************************"
...
function start_local_container() {
    ...
    then
        ...
        -v \"$(dirname "${SCRIPT_DIR}"):${LINK_DIR}${mount_opts}\" ${MOUNT_POINTS} \
    ...
    CONTAINER_GID="$(docker exec "${CONTAINER_NAME}" bash -c "id ${CONTAINER_USER} -g")"
    CONTAINER_UID="$(docker exec "${CONTAINER_NAME}" bash -c "id ${CONTAINER_USER} -u")"
...
while getopts "e:n:u:i:d:m:p:fvxh" opt; do
    case "${opt}" in
        n )
...
if ! docker info &> /dev/null; then
    exit_failure "unable to access Docker daemon: make sure ${CONTAINER_USER} is a member of the docker group"
fi
