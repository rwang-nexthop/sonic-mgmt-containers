    ...
    def critical_services_tracking_list(self):
        ...
        for service in list(self.sonichost.DEFAULT_ASIC_SERVICES):
            ...
            if service not in config_facts['FEATURE']:
                self.sonichost.DEFAULT_ASIC_SERVICES.remove(service)
                continue
            # Default to "False" if has_per_asic_scope is not present (for compatibility with older configs)
            if config_facts['FEATURE'][service].get('has_per_asic_scope', 'False') == "False":
                self.sonichost.DEFAULT_ASIC_SERVICES.remove(service)
